...
from utils.IRS_ct_channels_TNN_dbm import importData, add_noise
# import utils.IRS_ct_channels as IRS_ct_channels
from utils.complex_utils import turnReal, turnCplx, vec
from utils.NN_model.MLP_2layer_1024 import MLP
import os
from torch.optim.lr_scheduler import ReduceLROnPlateau
from utils.NN_model.TNN import TNN

def train(size, hidden_sizes, lr, num_epochs, batch_size, num_minibatch, cuda, snr, channel):
    ...
    def NMSEplot(epochs):
        ...
        plt.suptitle("nonparametric PD trained MMSE MIMO ChEst with TNN")
    ...
    h_test, _, _ = importData(test_size, device, phase = 'val', channel=channel)
    # h_test = h_test[int(2.4e4)-test_size:]
    ...
    ## construct TNN & NP MLP ## initialize \tbh
    tnn = TNN(n_T, n_I, n_R).to(device)
    print(torch.exp(1j * tnn.state_dict()['phase']))
    # for name, param in tnn.named_parameters():
    #     print(name, param.shape)
    logits_net = MLP(2*n_R*T, hidden_sizes[0], 2*n_R*n_T*n_I+2).to(device)
    # print(logits_net)
    optimizer_pri = Adam(list(logits_net.parameters()) + list(tnn.parameters()), lr=lr)
    ...
    for i in range(num_epochs):
        ...
        for j in range(num_minibatch):
            ...
            ### feed into the NN
            logits_net.train()
            tau_y = add_noise(tnn(turnCplx(tau_h)), SNR_lin, batch_size, n_R, T, device, itr, 'train' ,h_mean, h_std) 
            logits = logits_net(turnReal(tau_y))
            ...
            with torch.no_grad():
                ...
                # tau_y = adding_noise(tnn(turnCplx(tau_h)), SNR_lin, batch_size, n_R, T, device) 
                # logits = logits_net(turnReal(tau_y))

                # print(itr)
                y_test = add_noise(tnn(turnCplx(h_test)), SNR_lin, test_size, n_R, T, device, itr, 'val', h_mean, h_std)
            ...
            if itr == 1000:
                print(torch.exp(1j * best_tnn.state_dict()['phase']))
    ...
    print("best_itr:",best_itr)

    script_dir = os.path.dirname(os.path.abspath(__file__))
    save_path = os.path.join(script_dir, 'trained_model', '%.3f_SP_%s_MLP_TNN_lr%.0e_%s_ep%s' 
            %(best_loss, channel, lr, [2*n_R*T]+hidden_sizes+[2*n_R*n_T*n_I+2], train_epochs))
    lossPlot()
    # print(torch.exp(1j * best_tnn.state_dict()['phase']))
    Psi = torch.exp(1j * best_tnn.state_dict()['phase'])
    parametersSave()
...
if __name__ == '__main__':
    ...
    train([args.nT, args.nI, args.nR, args.T], args.hsz, args.lr, args.ep, args.mbs, args.nmb, args.cuda, args.snr, args.ch)
