...
def import_data(data_size, n_R, n_I, n_T, T, SNR_lin, device, IRScoef='identity', phase = 'train', channel='default'):
    ...
    BI_file_name = channel.lower() + '_BI_' + phase_ + '_36_64_ori_.mat'
    IU_file_name = channel.lower() + '_IU_' + phase_ + '_64_4_ori_.mat'
    
    dataset_folder = os.path.abspath(os.path.join(current_path, '..', '..', 'channel_realization_36_64_4'))
    for idx, file_name in enumerate([BI_file_name, IU_file_name]):
        ...
        with h5py.File(dataset_file_path, 'r') as f:
            # h = f['H_samples']
            h = torch.complex(torch.tensor(f['H_samples']['real']), torch.tensor(f['H_samples']['imag'])).to(device)
        ...
        if   idx == 0: H_bi = h.to(torch.complex64).permute(2,0,1)
        elif idx == 1: H_iu = h.to(torch.complex64).permute(2,0,1)

    H_c = batch_khatri_rao(H_bi, H_iu)#*10
