...
def train(size, hidden_sizes, lr, num_epochs, batch_size, num_minibatch, cuda, snr, channel='UMa', steering='x'):
    ...
    h, y, h_mean, h_std = importData(train_size, 2, 2, 6, 6, T, SNR_lin, device, 'train', channel, steering)
    h_test, y_test, _, _ = importData(test_size, 2, 2, 6, 6, T, SNR_lin, device, 'val', channel, steering)
    ...
    save_path = os.path.join(script_dir, 'trained_model', '%.3f_PD_%s_MLP_str_%s_lr%.0e_%s_ep%s' 
...
if __name__ == '__main__':
    ...
    train([args.nT, args.nR, args.T], args.hsz, args.lr, args.ep, args.mbs, args.nmb, args.cuda, args.snr, args.ch, args.str)
