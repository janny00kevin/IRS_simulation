...
def generate_steering_precoder(n_x, n_y, theta, phi, num_rf_chains, device):
    ...
    f_rf_1 = steering_vector(n_x, n_y, delta_x, delta_y, theta, phi)
    F_rf = torch.tile(f_rf_1.reshape(-1, 1), (1, num_rf_chains))
    ...
    # s = torch.zeros(num_rf_chains, dtype=torch.complex64)
    # s[1] = 1

    # print((F_rf@F_bb@s).shape)

    # plot_beam_pattern(calculate_beam_pattern((F_rf@F_bb)[:,20], n_x, n_y))


    return F_rf.to(device).to(torch.complex64), F_bb.to(device).to(torch.complex64)
