...
from utils.ct_n2n_channels_ano_36_4_precoder import importData
from utils.complex_utils import turnReal, turnCplx, vec
from utils.NN_model.MLP_2layer_1024 import MLP
import os
from torch.optim.lr_scheduler import ReduceLROnPlateau 

def train(size, hidden_sizes, lr, num_epochs, batch_size, num_minibatch, cuda, snr, channel='UMa', steering='x', num_data_stream=[1,1]):
    ...
    def NMSEplot(epochs):
        ...
        plt.suptitle("nonparametric PD trained MMSE MIMO ChEst with tx steering %s" %steering)
        plt.title(' $[n_R,n_I,n_T,T]$:[%s,%s,%s], lr:%s, size:%s, SNR:%s~%s' 
    ...
    h, y, h_mean, h_std = importData(train_size, 2, 2, 6, 6, T, SNR_lin, device, 'train', channel, num_data_stream, steering)
    h_test, y_test, _, _ = importData(test_size, 2, 2, 6, 6, T, SNR_lin, device, 'val', channel, num_data_stream, steering)
    ...
    logits_net = MLP(2*n_R*T, hidden_sizes[0], 2*n_R*n_T+2).to(device)
    print(logits_net)
    ...
    save_path = os.path.join(script_dir, 'trained_model', '%.3f_PD_%s_MLP_rxbf_%s_lr%.0e_%s_ep%s' 
            %(best_loss, channel, steering, lr, [2*n_R*T]+hidden_sizes+[2*n_R*n_T+2], train_epochs))
    lossPlot()
    parametersSave()
...
if __name__ == '__main__':
    ...
    parser.add_argument('-str', type=str, default='x') # steering vector ['aligned', 'a'], ['null', 'n'], ['omni', 'o']
    parser.add_argument('-nds', type=int, default='2')  # num of data streams, 1 or others
    ...
    if args.nds == 1:
        ...
        num_data_stream = [args.nT, args.nR]
    
    train([args.nT, args.nR, args.T], args.hsz, args.lr, args.ep, args.mbs, args.nmb, args.cuda, args.snr, args.ch, args.str, num_data_stream)
