...
import os  
from utils.ct_n2n_channels_ano_36_4_precoder import importData, steering_precoder_ano
...
T = n_T
# num_data_stream = [1,1]
num_data_stream = [n_T,n_R]

# MLPs
filename1 = os.path.join(script_dir, 'trained_model', '0.743_PD_uma_MLP_rxbf_a_lr1e-03_[288, 1024, 290]_ep74.pt')
checkpoint1 = torch.load(filename1, weights_only=False)
filename2 = os.path.join(script_dir, 'trained_model', '0.815_PD_uma_MLP_rxbf_n_lr1e-03_[288, 1024, 290]_ep59.pt')
checkpoint2 = torch.load(filename2, weights_only=False)
filename3 = os.path.join(script_dir, 'trained_model', '0.283_PD_uma_MLP_rxbf_o_lr1e-03_[288, 1024, 290]_ep86.pt')
...
h_test, y_test_a, h_mean, h_std = importData(test_size, n_R_x, n_R_y, n_T_x, n_T_y, T, SNR_lin, device, 'test', channel, num_data_stream, steering = 'a')
_, y_test_n, _, _ = importData(test_size, n_R_x, n_R_y, n_T_x, n_T_y, T, SNR_lin, device, 'test', channel, num_data_stream, steering = 'n')
_, y_test_o, _, _ = importData(test_size, n_R_x, n_R_y, n_T_x, n_T_y, T, SNR_lin, device, 'test', channel, num_data_stream, steering = 'o')
...
y_test_a = y_test_a.reshape(len(SNR_lin), datasize_per_SNR, num_data_stream[1]*T*2)
y_test_n = y_test_n.reshape(len(SNR_lin), datasize_per_SNR, num_data_stream[1]*T*2)
y_test_o = y_test_o.reshape(len(SNR_lin), datasize_per_SNR, num_data_stream[1]*T*2)
...
def LS_LMMSE(device, datasize_per_SNR, n_R, n_T, T, snr, h_test, y_test, num_data_stream, steering='x'):
    """
    ...
    ### LMMSE
    if num_data_stream[0] == 1:
        X_mat = torch.ones(1, T).to(device).to(torch.complex64)
    else:
        X_mat = torch.eye(n_T).to(device).to(torch.complex64)
    F_t = steering_precoder_ano(n_T_x, n_T_y, device, steering, num_data_stream[0])
    tbX_b4_Fr = batch_kronecker(X_mat.T@F_t.T, torch.eye(n_R).to(device).to(torch.complex64))
    Sgnl_b4_Fr = (tbX_b4_Fr@turnCplx(h_test).unsqueeze(2)).squeeze(2)
    Ps = (Sgnl_b4_Fr.abs()**2).mean(dim=0)
    Pn_b4_Fr = Ps / snr
    cov_n_b4_Fr = torch.diag(Pn_b4_Fr)
    # print(cov_n_b4_Fr.dtype)
    F_r = steering_precoder_ano(n_R_x, n_R_y, device, steering, num_data_stream[1])
    tbFr_H = batch_kronecker(torch.eye(T).to(device), F_r.conj().T) # .reshape(-1, F_r.shape[0]) = .T but keep memory contiguous
    # print(F_r.conj().T@F_r)
    # print(tbFr_H@tbFr_H.conj().T)
    cov_n = tbFr_H.to(torch.float32) @ cov_n_b4_Fr @ tbFr_H.conj().T.to(torch.float32)
    tbX = tbFr_H @ tbX_b4_Fr
...
for idx, snr in enumerate(SNR_lin):
    ...
    with torch.no_grad():
        ...
        norm_LM_a = LS_LMMSE(device, datasize_per_SNR, n_R, n_T, T, snr, h_test[idx], y_test_a[idx], num_data_stream, steering='a')
        norm_LM_n = LS_LMMSE(device, datasize_per_SNR, n_R, n_T, T, snr, h_test[idx], y_test_n[idx], num_data_stream, steering='n')
        norm_LM_o = LS_LMMSE(device, datasize_per_SNR, n_R, n_T, T, snr, h_test[idx], y_test_o[idx], num_data_stream, steering='o')
...
plt.plot(SNR_dB, NMSE_1.to('cpu'), label='NP PD w/ align', linewidth=1, linestyle='-', marker='x', color="tab:orange")   ###
plt.plot(SNR_dB, NMSE_2.to('cpu'), label='NP PD w/ null', linewidth=1, linestyle=':', marker='x', color="tab:orange")  ###
plt.plot(SNR_dB, NMSE_3.to('cpu'), label='NP PD w/ omni', linewidth=1, linestyle='--', marker='x', color="tab:orange")  ###
...
save_path = os.path.join(script_dir, 'ChEsts_testing_performance', 'beam_align_%s_ano_precoder_23p1_ds%i_.pdf'%(channel, num_data_stream[0])) #  %(IRS_coef_type)
