...
def importData(data_size, n_R_x, n_R_y, n_T_x, n_T_y, T, SNR_lin, device, phase = 'train', channel='x', num_data_stream = [0,0], steering='x'):
    """
    ...
    sgnl_channel = vec(H_bi @ F_t @ X_mat) # Transmitted signal (vectorized)
    ...
    # kron_combiner = torch.kron(torch.eye(T).to(device), F_r.conj().T)
    tbFr_H = batch_kronecker(torch.eye(T).to(device), F_r.conj().T) # .reshape(-1, F_r.shape[0]) = .T but keep memory contiguous
    y = (tbFr_H.unsqueeze(0) @ y_ant.unsqueeze(-1)).squeeze(-1) # Received signal (vectorized)
    # print(torch.isnan(y))
    # print((y.abs()**2).mean())
...
def steering_precoder_ano(n_x, n_y, device, steering, num_data_stream):
    # print(num_data_stream)
    match steering.lower():
        case 'a' | 'aligned':
            F_mat = generate_steering_precoder(n_x, n_y, torch.tensor(23.1), torch.tensor(0), num_data_stream, device)
        case 'n' | 'null':
            F_mat = generate_steering_precoder(n_x, n_y, torch.tensor(3.2), torch.tensor(0), num_data_stream, device)
        case 'o' | 'omni':
            if num_data_stream == 1:
                F_mat = torch.zeros(n_x*n_y, 1).to(device).to(torch.complex64)
                F_mat[0] = 1
            else:
                F_mat = torch.eye(n_x*n_y).to(device).to(torch.complex64)
        case _:
            raise NameError(f"{steering} is not a valid steering vector name")
    return F_mat
