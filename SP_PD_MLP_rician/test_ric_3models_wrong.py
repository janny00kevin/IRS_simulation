...
# ISTA-Nets
filename7 = os.path.join(script_dir, 'result', '0.168_SP_ric_ISTA_psi_i_lr1e-04_[256, 1, 258]_ep24.pt')
checkpoint7 = torch.load(filename7, weights_only=False, map_location=device1)
filename8 = os.path.join(script_dir, 'result', '0.301_SP_ric_ISTA_psi_d_lr1e-04_[256, 1, 258]_ep100.pt')
checkpoint8 = torch.load(filename8, weights_only=False, map_location=device1)
filename9 = os.path.join(script_dir, 'result', '0.214_SP_ric_ISTA_psi_h_lr1e-04_[256, 1, 258]_ep100.pt')
...
for idx, snr in enumerate(SNR_lin):
    ...
    ### ISTA-Net
    [logits7, error] = logits_net7((turnReal(turnCplx(y_test_i[idx])-h_mean)/h_std).to(device1))
    test_tbh_cplx = turnCplx(logits7).to(device)*h_std + h_mean
    norm_7 = torch.norm(turnCplx(h_test[idx]) - test_tbh_cplx, dim=1)**2
    
    [logits8, error] = logits_net8((turnReal(turnCplx(y_test_d[idx])-h_mean)/h_std).to(device1))
    test_tbh_cplx = turnCplx(logits8).to(device)*h_std + h_mean
    norm_8 = torch.norm(turnCplx(h_test[idx]) - test_tbh_cplx, dim=1)**2
    
    [logits9, error] = logits_net9((turnReal(turnCplx(y_test_h[idx])-h_mean)/h_std).to(device1))
...
save_path = os.path.join(script_dir, 'snr', 'SP_ric_MLP_vs_CN_IN_.pdf') #  %(IRS_coef_type)
