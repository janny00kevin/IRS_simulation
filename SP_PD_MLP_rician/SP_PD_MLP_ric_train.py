...
def train(size, hidden_sizes=[64, 32], lr=1e-3, num_epochs = 3, 
    ...
    ## save the model and parameters
        ...
        save_path = os.path.join(script_dir, 'result', '%.3f_SP_ric_MLP_psi_%s_lr%.0e_%s_ep%s_3snr.pt' 
    ...
    def lossPlot():
        ...
        save_path = os.path.join(script_dir, 'result', '%.3f_SP_ric_MLP_psi_%s_lr%.0e_%s_ep%s_3snr.pdf' 
                    %(testing_loss[itr-1].item(), IRS_coe_type, lr, [2*n_R*T]+hidden_sizes+[2*n_R*n_T*n_I+2], train_epochs))
