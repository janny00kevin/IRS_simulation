...
def train(size, hidden_sizes=[64, 32], lr=1e-3, num_epochs = 3, 
        batch_size = 1e4, num_minibatch = 10, cuda = 1, snr = 0, channel = 'UMa', IRS_coe_type = 'i', num_layers=10):
    ...
    save_path = os.path.join(script_dir, 'result', '%.3f_SP_ric_ISTA_psi_%s_lr%.0e_%s_ep%s' 
            %(best_loss, IRS_coe_type, lr, [2*n_R*T]+hidden_sizes+[2*n_R*n_T*n_I+2], train_epochs))
    lossPlot()
    parametersSave()        
...
if __name__ == '__main__':
    ...
    parser.add_argument('-mbs', type=int, default=5e3)  # size of mini-batch
