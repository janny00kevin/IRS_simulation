...
# channelNets
filename4 = os.path.join(script_dir, 'result', '0.201_SP_ric_elbir_psi_i_lr1e-04_[256, 1, 258]_ep10.pt')
checkpoint4 = torch.load(filename4, weights_only=False, map_location=device)
filename5 = os.path.join(script_dir, 'result', '0.219_SP_ric_elbir_psi_d_lr1e-04_[256, 1, 258]_ep10.pt')
checkpoint5 = torch.load(filename5, weights_only=False, map_location=device)
filename6 = os.path.join(script_dir, 'result', '0.209_SP_ric_elbir_psi_h_lr1e-04_[256, 1, 258]_ep10.pt')
checkpoint6 = torch.load(filename6, weights_only=False, map_location=device)

# ISTA-Nets
filename7 = os.path.join(script_dir, 'result', '0.171_SP_ric_ISTA_psi_i_lr1e-04_[256, 1, 258]_ep24.pt')
checkpoint7 = torch.load(filename7, weights_only=False, map_location=device)
filename8 = os.path.join(script_dir, 'result', '0.370_SP_ric_ISTA_psi_d_lr1e-04_[256, 1, 258]_ep100.pt')
checkpoint8 = torch.load(filename8, weights_only=False, map_location=device)
filename9 = os.path.join(script_dir, 'result', '0.280_SP_ric_ISTA_psi_h_lr1e-04_[256, 1, 258]_ep100.pt')
...
logits_net7 = checkpoint7['logits_net'].to('cuda:1')
logits_net8 = checkpoint8['logits_net'].to('cuda:1') 
# logits_net9 = logits_net9.to(device)
# logits_net8 = ISTANet(5, device, n_R, n_I, n_T, T)
# logits_net8.load_state_dict(checkpoint8['logits_net'])
# # logits_net8 = move_model_to_device(logits_net8, device)
# print(logits_net8.device)
...
for idx, snr in enumerate(SNR_lin):
    ...
    ### ISTA-Net
    [logits7, error] = logits_net7((turnReal(turnCplx(y_test_i[idx])-h_mean)/h_std).to('cuda:1'))
    test_tbh_cplx = turnCplx(logits7).to(device)*h_std + h_mean
    norm_7 = torch.norm(turnCplx(h_test[idx]) - test_tbh_cplx, dim=1)**2
    
    [logits8, error] = logits_net8((turnReal(turnCplx(y_test_d[idx])-h_mean)/h_std).to('cuda:1'))
    test_tbh_cplx = turnCplx(logits8).to(device)*h_std + h_mean
    norm_8 = torch.norm(turnCplx(h_test[idx]) - test_tbh_cplx, dim=1)**2
    
    [logits9, error] = logits_net9((turnReal(turnCplx(y_test_h[idx])-h_mean)/h_std).to('cuda:2'))
...
save_path = os.path.join(script_dir, 'snr', 'SP_ric_MLP_vs_CN_IN.pdf')#  %(IRS_coef_type)
