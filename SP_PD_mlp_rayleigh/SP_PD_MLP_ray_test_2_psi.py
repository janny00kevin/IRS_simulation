...
filename2 = os.path.join(script_dir, 'result', '0.377_SP_ray_MLP_psi_h_lr1e-04_[256, 1024, 258]_ep500.pt')
...
for idx, snr in enumerate(SNR_lin):
    ...
    # h_test, y_test, _, _ = importData(datasize_per_SNR, n_R, n_I, n_T, T, snr, device, IRScoef=IRS_coef_type)
    # Psi = get_IRS_coef(IRS_coef_type,n_R,n_I,n_T,T).to(device)
    ...
    logits2 = logits_net2(y_test_h[idx])
...
plt.plot(SNR_dB, NMSE_2.to('cpu'), label='NP PD with Hadamard', linewidth=1, linestyle='-', marker='x', color="tab:orange")  ###
# plt.plot(SNR_dB, NMSE_3,'-x', label='channelNet w/o act. func.')  ###
# plt.plot(SNR_dB, NMSE_4,'-x', label='channelNet w/ act. func.')  ###

plt.suptitle("NP PD trained MMSE MIMO ChEst with IRS vs SNR in Rayleigh")
...
save_path = os.path.join(script_dir, 'snr', 'SP_ray_i_h_vs_LS_LMMSE_MLP_100nmb.pdf')#  %(IRS_coef_type)
