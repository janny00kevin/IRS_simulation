...
def train(size, hidden_sizes, lr, num_epochs, batch_size, num_minibatch, cuda, snr, channel='UMa', IRS_coe_type='h', angle_config='x'):
    ...
    save_path = os.path.join(script_dir, 'trained_model', '%.3f_PD_%s_MLP_psi_%s%s_lr%.0e_%s_ep%s' 
            %(best_loss, channel, IRS_coe_type, angle_config, lr, [2*n_R*T]+hidden_sizes+[2*n_R*n_T*n_I+2], train_epochs))
    lossPlot()
    parametersSave()
...
if __name__ == '__main__':
    ...
    train([args.nT, args.nI, args.nR, args.T], args.hsz, args.lr, args.ep, args.mbs, args.nmb, args.cuda, args.snr, args.ch, args.psi, args.conf)
