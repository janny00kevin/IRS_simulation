...
from utils.IRS_ct_channels_8snr_mse import importData
...
if channel == 'inf':
    ...
    # filename2 = os.path.join(script_dir, 'trained_model', '1.282_8SNR_inf_5MLP_psi_h_lr5e-05_[256, 1024, 258]_ep129.pt')
    ...
    # filename1 = os.path.join(script_dir, 'trained_model', '4.646_eps1e+01_inf_AE_lr1e-03_[288, 1024, 288]_ep68.pt')
    # checkpoint1 = torch.load(filename1, weights_only=False, map_location=device)
    # filename2 = os.path.join(script_dir, 'trained_model', '0.720_8SNR_uma_5MLP_psi_h_lr1e-04_[256, 1024, 258]_ep170.pt')
    # checkpoint2 = torch.load(filename2, weights_only=False, map_location=device)
...
mse_MLP = torch.zeros_like(SNR_lin)
...
### import testing data ###
h_test, y_test_h, h_mean, h_std = importData(test_size, n_R, n_I, n_T, T, SNR_lin, device, IRScoef='h', phase = 'test', channel=channel, nmlz=False)
h_test2, y_test_h2, _, _ = importData(test_size, n_R, n_I, n_T, T, SNR_lin, device, IRScoef='h', phase = 'test', channel=channel, nmlz=True)
# print(h_mean, h_std)
...
h_test2 = h_test2.reshape(len(SNR_lin), datasize_per_SNR, n_R*n_T*n_I*2)
y_test_h = y_test_h.reshape(len(SNR_lin), datasize_per_SNR, n_R*T*2)
y_test_h2 = y_test_h2.reshape(len(SNR_lin), datasize_per_SNR, n_R*T*2)
# y_test_h = y_test_h.reshape(len(SNR_lin), datasize_per_SNR, n_R*T*2)
...
for idx, snr in enumerate(SNR_lin):
    ...
    with torch.no_grad():
        ...
        logits3 = logits_net3(y_test_h2[idx])
        test_tbh_cplx = turnCplx(logits3)
        norm_3 = torch.norm(turnCplx(h_test2[idx]) - D.matmul(test_tbh_cplx.T).T, dim=1)**2
        mse_MLP[idx] = norm_3.mean()
        ...
        NMSE_3[idx] = 10*torch.log10((norm_3 / torch.norm(h_test2[idx], dim=1)**2).mean())
...
# elif channel == 'inf':
#     plt.plot(SNR_dB, NMSE_2.to('cpu'), label='parametric PD AE $\\varepsilon=15$', linewidth=1, linestyle='-', marker='x', color="black")
...
save_path = os.path.join(script_dir, 'ChEsts_testing_performance', '%s_MLP_vs_CN_IN___.pdf'%(channel)) #  %(IRS_coef_type)
