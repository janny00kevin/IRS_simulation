...
from utils.IRS_ct_channels_8snr_ import importData
from utils.complex_utils_ import turnReal, turnCplx, vec
...
# MLPs
filename1 = os.path.join(script_dir, 'trained_model', '0.999_eps29.0_uma_AE_psi_h_lr1e-03_[256, 1024, 256]_ep119.pt')
checkpoint1 = torch.load(filename1, weights_only=False, map_location=device)
filename2 = os.path.join(script_dir, 'trained_model', '0.855_eps6.0_uma_AE_psi_h_lr1e-03_[256, 1024, 256]_ep183.pt')
...
filename6 = os.path.join(script_dir, 'trained_model/comaring_models', '0.652_SP_rt_elbir_psi_h_lr1e-04_ep20.pt')
...
filename9 = os.path.join(script_dir, 'trained_model/comaring_models', '0.709_SP_uma_ISTA_psi_h_lr1e-03_ep19.pt')
...
for idx, snr in enumerate(SNR_lin):
    ...
    with torch.no_grad():
        ...
        logits1 = turnCplx(logits_net1(turnReal(turnCplx(y_test_h[idx])-h_mean)/h_std)[0])*h_std + h_mean
        norm_1 = torch.norm(turnCplx(h_test[idx]) - logits1, dim=1)**2

        if idx == len(SNR_lin) - 1:
            latent_sparse = logits_net1(turnReal(turnCplx(y_test_h[idx])-h_mean)/h_std)[1]
            latent_cplx = torch.complex(latent_sparse[:, :128], latent_sparse[:, 128:])
            ...
            for m in range(datasize_per_SNR):
                ...
                if torch.all(temp == 0): 
                    count_all0 += 1
                count_nnz[m] = 128 - torch.count_nonzero(temp)
            print("avg sparsity",torch.mean(count_nnz).item()/128*100, "%")
            print("all zero count", count_all0/datasize_per_SNR*100, "%")

        logits2 = turnCplx(logits_net2(turnReal(turnCplx(y_test_h[idx])-h_mean)/h_std)[0])*h_std + h_mean
        norm_2 = torch.norm(turnCplx(h_test[idx]) - logits2, dim=1)**2

        if idx == len(SNR_lin) - 1:
            latent_sparse = logits_net2(turnReal(turnCplx(y_test_h[idx])-h_mean)/h_std)[1]
            latent_cplx = torch.complex(latent_sparse[:, :128], latent_sparse[:, 128:])
            count_nnz = torch.zeros(datasize_per_SNR, 1)
            count_all0 = 0
            for m in range(datasize_per_SNR):
...
plt.plot(SNR_dB, NMSE_6.to('cpu'), label='channelNet w/ H ', linewidth=1, linestyle='-', marker='o', color="tab:green")  ###
...
plt.plot(SNR_dB, NMSE_9.to('cpu'), label='ISTANet w/ H ', linewidth=1, linestyle='-', marker='o', color="tab:brown")  ###

plt.plot(SNR_dB, NMSE_3.to('cpu'), label='nonparametric MLP w/ H', linewidth=1, linestyle='-', marker='o', color="tab:orange")  ###
plt.plot(SNR_dB, NMSE_2.to('cpu'), label='PD AE $\\varepsilon=6$', linewidth=1, linestyle='-', marker='x', color="blue")  ###
plt.plot(SNR_dB, NMSE_1.to('cpu'), label='deeper PD AE $\\varepsilon=29$', linewidth=1, linestyle='-', marker='x', color="black")  ###
...
plt.suptitle("NP PD trained MMSE MIMO nmlz ChEst with IRS vs SNR in UMa28")
...
plt.grid(True)
data = {
    "SNR_dB": SNR_dB.to('cpu'), "LS w/ H": NMSE_LS_h.to('cpu'), "LMMSE w/ H": NMSE_LM_h.to('cpu'),
    "AE": NMSE_2.to('cpu'), "2 layers MLP w/ H": NMSE_3.to('cpu'), 
    "channelNet w/ H": NMSE_6.to('cpu'), "ISTANet w/ H": NMSE_9.to('cpu')
}
df = pd.DataFrame(data)
save_path = os.path.join(script_dir, 'ChEsts_testing_performance', 'SP_%s_MLP_vs_CN_IN_mulL_8snr_1.pdf'%(channel)) #  %(IRS_coef_type)
