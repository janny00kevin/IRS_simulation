...
from utils.NN_model.AE_MLP_1024 import SparseAutoencoder2
from torch.optim.lr_scheduler import ReduceLROnPlateau

def train(size, hidden_sizes, lr, num_epochs, batch_size, num_minibatch, cuda, snr, channel, IRS_coe_type, eps_fidel):
    ...
    def NMSE_plot(epochs):
        ...
        plt.suptitle("parametric PD trained AE MIMO ChEst with IRS coefficient %s" %IRS_coe_type)
        plt.title(' $[n_R,n_I,n_T,T]$:[%s,%s,%s,%s], lr:%s, size:%s, SNR:%s~%s' 
                  %(n_R,n_I,n_T,T,lr, [2*n_R*T]+hidden_sizes+[2*n_R*n_T*n_I], torch.min(SNR_dB).item(), torch.max(SNR_dB).item()))
    ...
    logits_net = SparseAutoencoder2(2*n_R*T, hidden_sizes[0], 2*n_R*n_T*n_I).to(device)
    # print(logits_net)
    num_params = sum(p.numel() for p in logits_net.parameters() if p.requires_grad)
    optimizer_pri = Adam(logits_net.parameters(), lr=lr)
    scheduler = ReduceLROnPlateau(optimizer_pri, mode='min', patience=3, factor=0.5, threshold=1e-7)
    ...
    for i in range(num_epochs):
        ...
        for j in range(num_minibatch):
            ...
            with torch.no_grad():
                ...
                if current_lr < lr and (testing_loss[itr-1] < best_loss) and KKT_termination(logits_net, constraint_loss, lamb_rec[itr-1], KKT_thres): # and lr_update_count > 0 
                    # iter_loss = iter_loss[0:i*num_minibatch+j]#torch.zeros(num_epochs*num_minibatch).to(device)
                    # testing_loss = testing_loss[0:i*num_minibatch+j]
                    # # t_rec = t_rec[0:(i+1)*num_minibatch]
                    # lamb_rec = lamb_rec[0:i*num_minibatch+j]
    ...
    save_path = os.path.join(script_dir, 'trained_model', '%.3f_eps%.1f_%s_AE_psi_%s_lr%.0e_%s_ep%s' 
            %(best_loss, eps_fidel, channel, IRS_coe_type, lr, [2*n_R*T]+hidden_sizes+[2*n_R*n_T*n_I], train_epochs))
    loss_plot()
    parameters_save()
...
if __name__ == '__main__':
    ...
    train([args.nT, args.nI, args.nR, args.T], args.hsz, args.lr, args.ep, args.mbs, args.nmb, args.cuda, args.snr, args.ch, args.psi, args.eps)
