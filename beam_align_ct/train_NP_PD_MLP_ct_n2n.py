...
def train(size, hidden_sizes, lr, num_epochs, batch_size, num_minibatch, cuda, snr, channel='UMa', steering='x'):
    ...
    save_path = os.path.join(script_dir, 'trained_model', '%.3f_PD_%s_MLP_str_%s_%s_lr%.0e_%s_ep%s' 
            %(best_loss, channel, steering, angle_config, lr, [2*n_R*T]+hidden_sizes+[2*n_R*n_T+2], train_epochs))
    lossPlot()
    parametersSave()
